    ...
    if history_list:
        for value in history_list[-1::-1]:
            ...
            history_result += ("<div class='form-signin form-history'><span class='history-info-title'>" + value[0] 
                + "</span><br /><span>http://people.ischool.berkeley.edu/~rahul.jverma/server/short/" + value[1] 
                + "</span><input type='button' class='btn btn-small btn-primary copybtn-xsmall copybutton' data-clipboard-text=" + "http://people.ischool.berkeley.edu/~rahul.jverma/server/short/" + value[1] + " value='Copy'/><span class='history-info'>"
                + value[3] + "</span></div>")
    if history_result == "":
...
def home():
    	...
    	if user_id is None:
        	...
        	resp.set_cookie('user_id', user_id, expires=expiresTime, path='/~rahul.jverma/server')
    	return resp
        ...
        for value in history_list:
            if regex.search(value[3].lower()):
                search_result += ("<div class='form-signin form-history'><span class='history-info-title'>" + value[0] 
                    + "</span><br /><span>http://people.ischool.berkeley.edu/~rahul.jverma/server/short/" + value[1] 
                    + "</span><input type='button' class='btn btn-small btn-primary copybtn-xsmall copybutton' data-clipboard-text=" + "http://people.ischool.berkeley.edu/~kikiliu/server/short/" + value[1] + " value='Copy'/><span class='history-info'>"
                    + value[3] + "</span></div>")
    if search_result == "":
